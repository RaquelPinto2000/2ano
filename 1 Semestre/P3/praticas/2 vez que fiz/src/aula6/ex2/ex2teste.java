...
public class ex2teste {
	public static void main(String[] args) {
		...
		List<Figura> ret = ListProcess.filter(lista, f -> f.area() > 20);
		printList(ret);
		System.out.println("\nFiguras Filter 2:");
		ret = ListProcess.filter(lista, f -> f.perimetro() < 15);
		printList(ret);
		System.out.println("\nFiguras Filter 3:");
		ret = ListProcess.filter(lista, f -> f.perimetro() < 15 && f.area() > 10);
		printList(ret);
		...
		System.out.println("\nEstudante Filter 1:");
		List<Estudante> ret2 = ListProcess.filter(lista2, e -> e.nMec() < 103);
		printList(ret2);
				
		System.out.println("\nEstudante Filter 2:");
		ret2 = ListProcess.filter(lista2,e -> e.getClass().getSimpleName().equals("Bolseiro"));
		printList(ret2);		
	}
