...
public class P3_19 { 	
	...
	public static void main(String[] args) throws FileNotFoundException, IOException {
		...
		PrintStream fl = new PrintStream(new File("Exame_Normal/resources/p3_1920P.txt"));
	...
	private static void alinea2(Banco b, PrintStream out) throws IOException {
		...
		// Adicione a seguir o código necessário para ler e processar o ficheiro com os movimentos de conta (usar java streams)
		 ...
		 List <String> lerfich = Files.readAllLines(Paths.get("Exame_Normal/resources/movimentos.txt")); // ou 	Files.readAllLines(Path.of("\"Exame_Normal/resources/movimentos.txt\""));
        for (int i = 0; i < lerfich.size(); i++) {
			String s[] = lerfich.get(i).split("");
			
			try{
				if(s[0].equals("C")) {
					b.add(Integer.parseInt(s[1]), new Credito (Integer.parseInt(s[2]),s[3]));
				}else if(s[0].equals("D")) {
					b.add(Integer.parseInt(s[1]), new Debito (-1 * Integer.parseInt(s[2]),s[3]));
				}
			}catch(Exception e) {
				 e.printStackTrace();
			}
	...
	import static src.Cliente.TipoCliente;  (src e a package dele)
	*/
   ...
   */
        ...
        try (FileWriter fw = new FileWriter(new File("Exame_Normal/resources/contasComSaldoNegativo.txt"))) {
		...
		out.println("\n----- Saldo total das contas eletrónicas -----");
		 ...
		 double saldoTotalOnline = b.getContas().stream().filter(e -> e instanceof Eletronica).map(Conta::getSaldo).reduce(0.0, Double::sum);
	        ...
	        try (FileWriter fw = new FileWriter(new File("Exame_Normal/resources/saldoTotalOnline.txt"))) {
