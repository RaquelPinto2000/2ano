...
void _int_(24) isr_uart1(void){
    if(IFS0bits.U1RXIF){
        ...
        U1STAbits.OERR =0;
        printf("jwkennwefnwefnenfenonrg\n");
        }else{ //else read U1RXREG to a dummy variable and clear UART1 error interrupt flag
    ...
    if(U1RXREG == 'L'){
        puts("\nValor Minimo: ");
        char unidades = 0x30 | ((voltageMin & 0xF0) >>4);
        char dezenas = 0x30 | ((voltageMin & 0x0F));
        putc(unidades);
        putc(dezenas);
        puts("\nValor Maximo: ");
        unidades = 0x30 | ((voltageMax & 0xF0) >>4);
        dezenas = 0x30 | ((voltageMax & 0x0F));
        putc(unidades);
        putc(dezenas);
        // Send "voltageMin" and "voltageMax" to the serial port UART1
        U1RXREG = voltageMin;
        U1RXREG = voltageMax;
