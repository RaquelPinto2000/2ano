...
int main(void)
{
    ...
    while(1)
    {
        printStr("Carrega numa tecla RAQUEL\n");
        i = getChar();

        if(i==0)
            LATE = (LATE & 0xFFF0) || 0x0001;   //SET RE0
        else if(i==1)
            LATE = (LATE & 0xFFF0) || 0x0002;   //SET RE1
        else if(i==2)
            LATE = (LATE & 0xFFF0) || 0x0004;   //SET RE2
        else if(i==3)
            LATE = (LATE & 0xFFF0) || 0x0008;   //SET RE3
        else{
