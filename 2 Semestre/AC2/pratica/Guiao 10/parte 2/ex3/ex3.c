...
volatile int voltage = 0x00;     //global variable
int temperature;
...
void configureAll(){
  ...
  TRISB = TRISB & 0X80FF;//configurar RB15 to RB8 como outputs; (RB15 = ponto)
  LATB = LATB & 0X80FF;
...
int main(void){
    ...
    while(1){
      ...
      portVal = PORTB & 0x0003; // vai buscar a entrada do RB1 e RB0
      switch (portVal) {
