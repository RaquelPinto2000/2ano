...
public class SemanticAnalysis extends analiseDimensionalBaseVisitor<Boolean> {
   ...
   public Boolean visitIfCond(analiseDimensionalParser.IfCondContext ctx) {
      Boolean res = visit(ctx.expr()) && visit(ctx.program(0));
      if (res) {
         if (!"boolean".equals(ctx.expr().ti.name())) {
            ErrorHandling.printError(ctx, "OOPS: Boolean expression required in conditional instruction!");
            res = false;
         }
   ...
   @Override
   public Boolean visitExprNot(analiseDimensionalParser.ExprNotContext ctx) {
      Boolean res = visit(ctx.expr());
      if (res) {
   ...
   public Boolean visitBoolLiteral(analiseDimensionalParser.BoolLiteralContext ctx) {
     /* Boolean res = null;
      ...
      if (res) {
         if (fetchType(ctx.ti, boolType) == null) {
            ErrorHandling.printError(ctx, "OOPS: Invalid Type!");
            res = res && false;
         } else {
            ctx.ti = boolType;
         }
   ...
   public Boolean visitExprReal(analiseDimensionalParser.ExprRealContext ctx) {
      ...
      /*Boolean res = null;
      VariableSymbol vs = new VariableSymbol(ctx.var, ctx.ti);

      if (fetchType(ctx.ti, realType) == null) {
         ErrorHandling.printError(ctx, "OOPS: Invalid Type!");
         res = false;
      } else {
         ctx.ti = realType;
   ...
   public Boolean visitExprInteger(analiseDimensionalParser.ExprIntegerContext ctx) {
      ...
      if (fetchType(ctx.ti, intType) == null) {
         ErrorHandling.printError(ctx, "OOPS: Invalid Type!");
         res = false;
      } else {
         ctx.ti = intType;
