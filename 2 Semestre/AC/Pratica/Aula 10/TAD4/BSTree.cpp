...
// A: Sim, porque antes de remover-mos um nó pai devemos excluir primeiro os nós filhos
...
static int _treeGetHeight(const struct _BSTreeNode* root) {
  ...
  int esq = _treeGetHeight(root->left);
  int dir = _treeGetHeight (root->right);
  if(esq<dir){
    return 1+dir;
  }
  return 1+esq;
...
int BSTreeContains(const BSTree* header, const void* item) {
  ...
  while (root != NULL) {
    ...
    // ...
    int comp = header->compare(root->item, item);
    if(comp==0){
      return 1;
    }
    if(comp>0){
...
_getPointerToKthNode(struct _BSTreeNode* root, unsigned int k) {
  ...
  // O erro era k < numNodesLeftTree passa a k > numNodesLeftTree
...
// R: Como esta árvore tem n nós e está equilibrada (balanced), concluimos
// que o esforço computacional depende do comprimento do caminho a partir
// da raiz da árvore, logo a ordem de complexidade desta função é logarítmica
// O(log(n)) para assegurar um bom "comportamento".

// Explique como poderia evitar a chamada a _getNumNodes nesta função.
// Q: O que seria preciso mudar na estrutura de dados?
// R: Como a árvore tem n nós e está equilibrada (balanced) para evitar a chamada
// a _getNumNodes nesta função bastaria adicionar uma variavel int height à
// struct _BSTreeNode que devolveria o respetivo valor da altura

// Q: E que funções teriam de ser modificadas?
// R: Se fizessemos as alterações mensionadas em cima, teriamos que alterar as
// funções BSTreeAdd, BSTreeRemove, BSTreeCreate e _treeDestroy
...
// R: Se usar-mos uma função recursiva para remover o nó, percorreremos os
// nós apenas uma vez, assim implementavamos um modo mais eficiente.
