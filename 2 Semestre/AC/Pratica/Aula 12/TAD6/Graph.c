...
Graph* GraphFromFile(FILE* f) { // ver isto.------
  // COMPLETAR !!
  //
/*
  unsigned short isDigraph;
  unsigned short isWeighted;
  int numVertices;
  if(fscanf(f,"%uhd %uhd %d %d",&isDigraph,&isWeighted,&numVertices,&numEdges)!=4){
    exit();   //erro
  }

  GraphCreate();
  for(int i = 0;i<numEdges;i++){
    if(fscanf(f,"%d %d %f",,,num)==3){
        int num = (num) * 10000;
        GraphAddEdge();
    }else{
      if(fscanf(f,"%d %d")==2){
  ...
  //if(fscanf(f,"%d")==0){ //ver se ja chegaste ao final do ficheiro
...
unsigned short GraphCheckInvariants(const Graph* g) {
  // COMPLETAR !!
  assert(g->numVertices == ListGetSize(g->verticesList)); // este estava no add foi copiado. E preciso?
  assert(g->isDigraph!=0 && g->isWeighted!=0 && g->numVertices!=0);
  assert(g->isComplete ==0 && g->numEdges == 0);
