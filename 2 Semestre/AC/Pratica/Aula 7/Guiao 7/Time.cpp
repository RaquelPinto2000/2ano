...
// Getters
// devolve o numero de horas
int TimeGetHH(Time t) {
  return (int)t/3600;
}
// devolve o numero de minutos
int TimeGetMM(Time t) {
  int res = t%3600;
  return res/60;
}
// devolve o numero de segundos
...
Time TimeParse(const char* str) {
  ...
  int r = sscanf(str, "%02d:%02d",&hh,&mm);
  if (n == 3 && TimeIsValid(hh, mm, ss)) {
    t = TimeCreate(hh, mm, ss);
  }else if (r==2 && TimeIsValid(hh,mm,0)&& n!=3){
...
Time TimeAdd(Time a, Time b) {
  ...
  Time t;
  if(a+b>TimeMAX){ // For example: adding 2:30:40 to 23:30:00 should return 02:00:40.
     t = (a+b)-(TimeMAX+1);
  }else{    // For example: adding 2:30:00 to 4:30:00 should return 7:00:00
    t = (a+b);
  }
...
Time TimeSubtract(Time a, Time b) {
  // KEEP IT SIMPLE, PLEASE!
  Time t;
  if(a-b<TimeMIN){ // For example: 2:30:00 minus 4:10:20 should return 22:19:40.
     t = (TimeMAX+1)-(b-a);
  }else{ // For example: 2:30:00 minus 1:10:00 should return 1:20:00
    t = (a-b);
  }
