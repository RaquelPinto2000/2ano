...
int DateCompare(const Date* a, const Date* b) {
  if(a->year - b->year != 0 ){ // quando sao diferentes
   return a->year - b->year;
  }else if(a->month- b->month){ // ano igual e mes diferente
     return a->month- b->month;
  }else{ // ano igual e mes igual, verificar para o dia
     return a->day - b-> day;
  }
}
...
void DateIncr(Date* d) {
  ...
  if(d->day == 31 && d-> month == 12){
		...
		d->month=1;
		d->day=1;
	} else if(d->day < DateDaysInMonth(d->year,d->month)){
		d->day = d->day + 1;
	}else if (d->day == DateDaysInMonth(d->year,d->month)){
		d->month = d->month + 1;
		d->day = 1;
