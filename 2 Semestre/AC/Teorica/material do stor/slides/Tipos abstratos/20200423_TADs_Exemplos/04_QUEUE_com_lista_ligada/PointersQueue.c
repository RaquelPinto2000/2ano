...
struct _PointersQueueNode {   // ver os nos da lista
  void* data;
...
struct _PointersQueue {
  ...
  struct _PointersQueueNode* tail;  // the tail of the Queue -> ultimo elemento
};
...
void* QueueDequeue(Queue* q) {  // retirar um elemento -> libertas a cabeça -> 1 elemento
  assert(q != NULL && q->size > 0);
