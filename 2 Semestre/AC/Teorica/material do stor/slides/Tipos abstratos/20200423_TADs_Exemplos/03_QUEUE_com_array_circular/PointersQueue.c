...
struct _PointersQueue {
  ...
  int head; // cabeça -> 1 elemento
  int tail; // rabo -> ultimo elemento
  void** data;  // the Queue data (pointers stored in an array)
};

// PRIVATE auxiliary function

static int increment_index(const Queue* q, int i) { // incrementar o indice -> nao pode ser maior que o espaco maximo
...
void QueueEnqueue(Queue* q, void* p) { // acrescentar um elemento
...
void* QueueDequeue(Queue* q) { // retirar um elemento
  assert(q->cur_size > 0);
  int old_head = q->head;
