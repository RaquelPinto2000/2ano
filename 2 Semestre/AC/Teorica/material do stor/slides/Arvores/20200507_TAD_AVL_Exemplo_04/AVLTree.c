...
static void _updateNodeHeight(AVLTree* t) { // consultar a altura das 2 sub-arvores para cada no e ajustar as arvores desse no
  assert(t != NULL);
...
// podes ver esta funcao nos apaontamntos do rapaz
...
static void _balanceNode(AVLTree** pRoot) {
  ...
  if (leftHeight - rightHeight == 2) { // se o criterio de ordem falhar na parte esquerda
  ...
  } else if (rightHeight - leftHeight == 2) {// se o criterio de ordem falhar na parte direita
...
int AVLTreeAdd(AVLTree** pRoot, const ItemType item) {
  ...
  if (root == NULL) { // se for o primeiro no da arvore (raiz)
  ...
  if (item < root->item) { // se o item a inserir for < que o item do no atual ele vai tentar inserir a esquerda do no atual
    // Try to insert on the left
    if (AVLTreeAdd(&(root->left), item) == 0) { // se nao resultar ele devolve 0
      // No success
      return 0;
    }

    // se a arvore nao estiver esquerda nao estiver balanciada
    // Unbalanced on the left ?
    if (AVLTreeGetHeight(root->left) - AVLTreeGetHeight(root->right) == 2) {
      if (item < root->left->item) { // se o item for menor que o item do no atual
        _singleRotateWithLeftChild(pRoot);// ele faz 1 rotacao a esquerda
      } else {
        _doubleRotateWithLeftChild(pRoot);// se nao ele faz dupla rotacao a esquerda
      }
    }
    _updateNodeHeight(root);
    return 1;
  } else if (item > root->item) { // se o item a inserir for > que o item do no atual ele vai tentar inserir a direita do no atual
    // Try to insert on the right
    if (AVLTreeAdd(&(root->right), item) == 0) {
      return 0;
    }
    // se a arvore nao estiver direita nao estiver balanciada
    ...
    _updateNodeHeight(root);
    return 1;
  } else { // se nao der nenhuma das anteriores e pq nao da para inserir
    // Not allowed
    return 0;
  }
}
